...
import {
  ConfirmarTestemunhaDto,
  ConvidarTestemunhaDto,
} from './dto/testemunha.dto';
...
export class ApuracaoService {
  ...
  async publicarResultados(adminId: string) {
    const edicao = await this.edicoesService.getEdicaoAtiva();
    ...
    for (const categoria of categorias) {
      ...
      if (!finalistas.length) continue;

      const ranking = [...finalistas].sort(
        (a, b) => b._count.votos - a._count.votos,
      );
    ...
    const linhas = publicados.map(
      (r) =>
        `- **${r.categoria.nome}**: ${r.finalista.inscricaoStartup.nomeStartup} (${r.totalVotos} votos)`,
    );
    ...
    const linhasTestemunhas = testemunhasConfirmadas.length
      ? [
          ...
          '',
          ...testemunhasConfirmadas.map(
            (t) =>
              `- **${t.nome}**${t.organizacao ? ` (${t.organizacao})` : ''} — ${t.email}`,
          ),
        ]
      : [];
    const conteudoMd = [
    ...
    await this.auditoria.registrar({
      edicaoId: edicao.id,
      tipo: TipoAuditoria.RESULTADO_PUBLICADO,
  ...
  async promoverFinalistasDoColegio() {
    ...
    for (const categoria of categorias) {
      ...
      const candidatos = ranked.map((r) => ({
        ...
        capital: this.edicoesService.isCapital(
          cidadeById.get(r.inscricaoStartupId),
          cidadesCapital,
        ),
      }));

      const selecionados = this.selecionarComCotas(
        candidatos,
        edicao.nFinalistasPorCategoria,
        edicao.cotasRegionaisAtivas,
        edicao.cotaMinCapital,
        edicao.cotaMinInterior,
      );
      ...
      for (const item of selecionados) {
        const finalista = await this.prisma.finalista.upsert({
          ...
          create: {
            ...
            categoriaId: categoria.id,
            inscricaoStartupId: item.inscricaoStartupId,
            metaResumo: `Selecionada pelo colégio (${item.apoios} apoios)${
              edicao.cotasRegionaisAtivas
                ? item.capital
                  ? ' · cota capital'
                  : ' · cota interior'
  ...
  /**
   * Garante mínimo de capital/interior quando cotas estão ativas,
   * preenchendo o restante pelos mais bem colocados.
   */
  private selecionarComCotas<
    T extends { inscricaoStartupId: string; apoios: number; capital: boolean },
  >(
    candidatos: T[],
    n: number,
    cotasAtivas: boolean,
    ...
    const take = (pred: (c: T) => boolean, need: number) => {
      for (const c of candidatos) {
        if (picked.length >= limit || need <= 0) break;
        if (used.has(c.inscricaoStartupId) || !pred(c)) continue;
        picked.push(c);
  ...
  async getAtaPublica() {
    ...
    if (!ata) throw new NotFoundException('Ata de apuração ainda não publicada.');
  ...
  async convidarTestemunha(dto: ConvidarTestemunhaDto) {
    const edicao = await this.edicoesService.getEdicaoAtiva();
  ...
  async getConviteTestemunha(token: string) {
    ...
    if (!testemunha) throw new NotFoundException('Convite de testemunha não encontrado.');
  ...
  async confirmarTestemunha(token: string, dto?: ConfirmarTestemunhaDto) {
    const testemunha = await this.prisma.testemunhaApuracao.findUnique({
      where: { token },
    });
    if (!testemunha) throw new NotFoundException('Convite de testemunha não encontrado.');
    if (testemunha.status === StatusTestemunhaApuracao.CONFIRMADA) {
      return testemunha;
    }
    if (testemunha.status === StatusTestemunhaApuracao.RECUSADA) {
      throw new BadRequestException('Este convite foi marcado como recusado.');
    }
  ...
  /** Galeria pública: vencedores e histórico por edição. */
  async galeriaVencedores() {
    const edicoes = await this.prisma.edicao.findMany({
      ...
      select: {
        ...
        testemunhas: {
          where: { status: StatusTestemunhaApuracao.CONFIRMADA },
          select: {
            nome: true,
            organizacao: true,
            confirmadaEm: true,
